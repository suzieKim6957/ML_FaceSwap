...
def swap_n_show_same_img(img1_fn,
    ...
    if plot_after:
        # random swap
        random_idx_list = [-1 for i in range(len(faces))]
        for i in range(len(faces)):
            idx_list = [idx for idx in range(len(faces)) if idx not in random_idx_list and idx != i]
            print(f'[{i}] {idx_list}')
            if len(idx_list) == 0:
                random_idx_list[i] = i
            else:
                random_idx_list[i] = random.choice(idx_list)
...
def swap_face_single(img1_fn, img2_fn, app, swapper,
    ...
    print(f'Image 1: {img1_fn}')
    print(f'Image 2: {img2_fn}')
